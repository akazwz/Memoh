...
if (store.status === 'idle') {
  ...
  else if (previous?.agent && ['codex', 'claude-code', 'opencode'].includes(previous.agent.agentId)) {
    store.restore({ botId: previous.botId, botName: '', displayName: '', agentId: previous.agent.botAgentId,
      runtime: previous.agent.agentId as 'codex' | 'claude-code' | 'opencode', authorizationId: previous.agent.authorizationId, setupError: null }, true)
  } else if (previous) {
...
async function goToBot() {
  ...
  if (props.onboarding && bot.value?.id) {
    ...
    writeOnboardingBotResult({ botId: bot.value.id, modelConfigured: modelConfigured.value,
      ...((runtime === 'codex' || runtime === 'claude-code' || runtime === 'opencode') && { agent: {
