...
-- existing codex/claude-code ACP agents, sessions, and bot credentials over
...
SELECT
  needed.team_id,
  needed.bot_id,
  CASE needed.provider WHEN 'codex' THEN 'Codex' ELSE 'Claude Code' END ||
    CASE WHEN EXISTS (
      SELECT 1 FROM bot_agents named
      WHERE named.team_id = needed.team_id
        AND named.bot_id = needed.bot_id
        AND lower(btrim(named.name)) = lower(CASE needed.provider WHEN 'codex' THEN 'Codex' ELSE 'Claude Code' END)
        AND named.deleted_at IS NULL
    ) THEN ' (migrated)' ELSE '' END,
  needed.provider,
  true,
