...
-- application rollback should normally retain the additive 0130 schema.
...
BEGIN
  ...
  -- Prevent a writer from inserting 0130-only data after the guard scan.
  ...
  IF has_options <> has_selected_option_id THEN
    RAISE EXCEPTION '0130 approval option columns are only partially present';
  END IF;
  ...
  ALTER TABLE public.tool_approval_requests
    ADD CONSTRAINT tool_approval_operation_check_down_0130
  ...
  IF has_options THEN
    ...
    ALTER TABLE public.tool_approval_requests
      ADD CONSTRAINT tool_approval_options_rollback_guard_0130
      CHECK (options = '[]'::jsonb AND selected_option_id = '') NOT VALID;
  END IF;

  BEGIN
    ALTER TABLE public.tool_approval_requests
      VALIDATE CONSTRAINT tool_approval_operation_check_down_0130;
    IF has_options THEN
      ALTER TABLE public.tool_approval_requests
        VALIDATE CONSTRAINT tool_approval_options_rollback_guard_0130;
    END IF;
  EXCEPTION
    WHEN check_violation THEN
      RAISE EXCEPTION 'cannot roll back 0130: tool approval rows contain permission choices or selections';
  END;

  ALTER TABLE public.tool_approval_requests
    DROP CONSTRAINT IF EXISTS tool_approval_operation_check;
  ALTER TABLE public.tool_approval_requests
    DROP CONSTRAINT IF EXISTS tool_approval_operation_check_0130;
  ALTER TABLE public.tool_approval_requests
    RENAME CONSTRAINT tool_approval_operation_check_down_0130 TO tool_approval_operation_check;
