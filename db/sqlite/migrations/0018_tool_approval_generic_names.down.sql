-- 0018_tool_approval_generic_names (down)
-- Restore the strict tool_name check. Rows using the synthetic
-- acp_permission[:kind] names cannot satisfy the narrow constraint and are
-- dropped, mirroring the postgres down migration. SQLite cannot alter a
-- CHECK constraint, so rebuild the table (same pattern as the up migration).
...
INSERT OR IGNORE INTO tool_approval_requests_old (
  id, bot_id, session_id, route_id, channel_identity_id, tool_call_id,
  tool_name, tool_input, short_id, status, decision_reason,
...
WHERE tool_name IN ('write', 'edit', 'exec');
