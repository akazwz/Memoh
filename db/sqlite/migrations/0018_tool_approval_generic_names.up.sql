-- 0018_tool_approval_generic_names
...
-- check to admit them. SQLite cannot alter a CHECK constraint, so rebuild the
-- table (same pattern as 0016).
...
CREATE TABLE IF NOT EXISTS tool_approval_requests_new (
  ...
  decided_at TEXT,
  CONSTRAINT tool_approval_tool_name_check CHECK (tool_name IN ('write', 'edit', 'exec') OR tool_name LIKE 'acp_permission%'),
...
INSERT OR IGNORE INTO tool_approval_requests_new (
