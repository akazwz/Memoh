# shellcheck shell=sh
# Install @xai-official/grok into a fresh versions/<version> directory, then switch `current`.
#
...
actual=$(GROK_HOME="$stage/bootstrap" "$stage/root/bin/$cmd" --no-auto-update --version 2>/dev/null) || actual=""
case "$actual" in
  "grok $ver" | "grok $ver "*) ;;
  *)
    dep_log "staged Grok executable version does not match $ver"
    rm -rf "$stage"
    exit 1
    ;;
esac
# Install the verified native executable itself. The npm wrapper otherwise
# decompresses a copy into every short-lived GROK_HOME at runtime.
cp "$stage/bootstrap/bin/grok-$ver" "$stage/root/bin/grok.native"
rm "$stage/root/bin/$cmd"
mv "$stage/root/bin/grok.native" "$stage/root/bin/$cmd"
if ! "$stage/root/bin/$cmd" --no-auto-update --version >/dev/null 2>&1; then
