...
当前第一阶段只做 Desktop/local workspace 的最小实现：默认用预装的 `codex-acp` 启动 Codex ACP adapter，本地缺少该命令时回退到 `npx -y @zed-industries/codex-acp`；不构建 workspace 镜像，不做 DB 级 task 持久化。目标是先验证 Memoh 能通过 ACP 启动 Codex 子会话、把 Codex 输出流式展示在同一个聊天里，并在该会话活跃时把后续用户消息路由给 Codex。所有文件和 terminal 操作都发生在 trusted local workspace 中。
...
- 第一阶段不构建 Codex workspace 镜像；`codex-acp` adapter 预装在 workspace toolkit 中，本地 trusted workspace 缺少该命令时兼容回退到 `npx -y @zed-industries/codex-acp`。
...
3. `internal/acpclient` 保持通用，不内置 Codex 命令；Codex profile 默认通过 workspace toolkit 中的 `codex-acp` 启动 adapter，本地 trusted workspace 可以回退到用户本机已有的 Node/npm `npx`。
4. 依赖用户已经完成 Codex 登录或配置好本机环境变量；第一阶段不从 Memoh provider/OAuth 系统注入 Codex 凭证。
5. 使用现有 workspace bridge `ExecStream` 启动 profile 传入的 ACP command，暂时不改 bridge proto，也不新增 `ExecStreamWithEnv`。
...
- `codex-acp` 通过 trusted local workspace 的 in-process bridge 启动，第一阶段由 Codex profile 提供启动命令。
- 如果本地 trusted workspace 的 PATH 中没有 `codex-acp`，会回退到 `npx -y @zed-industries/codex-acp`。
...
第一阶段通过 workspace toolkit 预装 `codex-acp` 的 Linux 平台包；Node.js 仍保留为通用 workspace toolkit 能力和本地 fallback 能力。
...
internal/acpclient/
  ...
  process.go       # 通过 workspace bridge stdio 启动 profile 传入的 ACP command
...
- 校验项目路径。
- 通过 bridge stdio 启动 Codex profile 提供的 ACP command。
...
- 通过 `internal/acpclient` 启动 profile command、创建 ACP session、发送初始 prompt。
...
- 不改 bridge proto。
- 使用现有 `ExecStream(ctx, command, workDir, timeout)` 启动 profile command。
- local bridge 会继承 `memoh-server` 进程环境；如果没有预装 `codex-acp`，fallback 使用的 `npx` 必须对该进程 PATH 可见。
...
4. 用现有 bridge `ExecStream` 启动 Codex profile command；Codex profile 内部优先执行 `codex-acp`，必要时回退到 `npx -y @zed-industries/codex-acp`。
...
- local workspace bot 可以通过 `codex_delegate` 调用 `codex-acp`。
- 缺少 `codex-acp` / `npx` fallback 或 adapter 启动失败时返回明确错误，提示用户确认 Desktop/local server PATH。
...
2. 第一阶段默认通过预装在 workspace toolkit 中的 `codex-acp` 启动 adapter。
3. 第一阶段不构建 workspace 镜像。
...
2. workspace PATH 中存在 `codex-acp`，或用户本机 PATH 中存在 `npx` fallback；Codex 已登录或可用。
...
7. 非 local workspace bot、缺少 `codex-acp` / `npx` fallback、非法 project path 都有明确错误。
8. 不需要 DB migration 或 workspace 镜像改动。

产品 MVP 完成时应满足：

1. 用户能在普通聊天里要求 Codex 处理 bot workspace 里的 repo。
2. Memoh 在 workspace 内启动 `codex-acp`，并通过 ACP 发送任务。
