...
func newTerminalManager(ctx context.Context, client *bridge.Client, root, defaultCwd string, timeoutSeconds int32) *terminalManager { //nolint:contextcheck // terminal streams must live for the ACP turn, not a single RPC callback.
	if timeoutSeconds <= 0 {
...
func (m *terminalManager) CreateTerminal(_ context.Context, p acp.CreateTerminalRequest) (acp.CreateTerminalResponse, error) {
	if len(p.Env) > 0 {
		return acp.CreateTerminalResponse{}, errors.New("terminal env is not supported by the first-stage ACP bridge")
	}
	cwd := m.defaultCwd
	if p.Cwd != nil && strings.TrimSpace(*p.Cwd) != "" {
		resolved, err := ResolvePathUnderRoot(m.root, *p.Cwd)
		if err != nil {
	...
	stream, err := m.client.ExecStream(m.ctx, command, cwd, m.timeout) //nolint:contextcheck // use the ACP turn context so terminal output survives the create RPC.
	if err != nil {
		return acp.CreateTerminalResponse{}, err
	}
