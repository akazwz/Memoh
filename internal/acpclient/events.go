...
func (c *eventCollector) apply(n acp.SessionNotification) {
	...
	case update.ToolCall != nil:
		tc := update.ToolCall
		c.upsertTool(string(tc.ToolCallId), tc.Title, string(tc.Status))
	case update.ToolCallUpdate != nil:
		...
		c.upsertTool(string(tc.ToolCallId), title, status)
...
func (c *eventCollector) upsertTool(id, title, status string) {
	if id == "" {
		return
	}
