...
// dependencyID is the catalog entry that provisions the native executable.
const dependencyID = "opencode"
...
func (d *Driver) resolveLauncher(ctx context.Context, botID string) (external.Launcher, error) {
	if d.launchers == nil {
		return external.Launcher{Path: "/opt/memoh/toolkit/bin/opencode", Source: external.LauncherSourceToolkit}, nil
	}
	...
	if err != nil {
		...
		return external.Launcher{}, apperror.Wrap(apperror.CodeExternalRuntimeUnavailable,
			fmt.Errorf("resolve opencode launcher: %w", err), map[string]string{"runtime": RuntimeType})
	}
	if strings.TrimSpace(launcher.Path) == "" {
		return external.Launcher{}, apperror.Wrap(apperror.CodeExternalRuntimeUnavailable,
			errors.New("resolve opencode launcher: resolver returned an empty path"), map[string]string{"runtime": RuntimeType})
	}
	return launcher, nil
}
...
func dependencyMissingFeedback(missing *external.DependencyMissingError) *agentfeedback.Error {
	message := "OpenCode is not installed in this workspace. Ask a bot administrator to install it from the bot's dependencies, then send the message again."
	operationInProgress := missing.OperationInProgress || strings.TrimSpace(missing.TaskID) != ""
	if operationInProgress {
		message = "OpenCode is not installed in this workspace yet; a dependency operation is already in progress. Send the message again when it finishes."
	}
...
func (d *Driver) observeVersion(ctx context.Context, botID, version string) {
	if observer := d.versionObserver(botID); observer != nil {
		observer(ctx, version)
	}
	if version != ProtocolVersion {
		d.logger.Warn("OpenCode version differs from verified protocol baseline", slog.String("version", version), slog.String("verified", ProtocolVersion))
	}
}
