...
/**
 * Abort the session's in-flight ACP turn
 *
 * Cancels the running turn out-of-band: works from any client or connection, not just the one that started the turn. The runtime stays warm and the round persists as cancelled. Pass turn_id to abort only that exact turn (a stale client then cannot kill a newer one).
 */
export const postBotsByBotIdSessionsBySessionIdAcpRuntimeAbortTurnMutation = (options?: Partial<Options<PostBotsByBotIdSessionsBySessionIdAcpRuntimeAbortTurnData>>): UseMutationOptions<PostBotsByBotIdSessionsBySessionIdAcpRuntimeAbortTurnResponse, Options<PostBotsByBotIdSessionsBySessionIdAcpRuntimeAbortTurnData>, PostBotsByBotIdSessionsBySessionIdAcpRuntimeAbortTurnError> => ({
    mutation: async (vars) => {
...
export const getBotsByBotIdSessionsBySessionIdAcpRuntimeTurnQueryKey = (options: Options<GetBotsByBotIdSessionsBySessionIdAcpRuntimeTurnData>) => createQueryKey('getBotsByBotIdSessionsBySessionIdAcpRuntimeTurn', options);

/**
 * Get the session's current ACP turn snapshot
 *
 * Reconnect backfill: the UI messages accumulated server-side for the in-flight (or most recent) turn, plus the turn identity for live-stream dedupe and abort targeting. active=false with no messages means no turn ran in this server process — persisted history is the whole story.
 */
export const getBotsByBotIdSessionsBySessionIdAcpRuntimeTurnQuery = defineQueryOptions((options: Options<GetBotsByBotIdSessionsBySessionIdAcpRuntimeTurnData>) => ({
    key: getBotsByBotIdSessionsBySessionIdAcpRuntimeTurnQueryKey(options),
    query: async (context) => {
